...
Spec_Gen_Input :: struct {
	...
	classes:                      []Class_Array,
	datatype_to_reg_kind:         [backend.Node_Datatype]backend.Reg_Kind,
	spill_boundary:               []int,
	cc_table:                     []backend.Call_Conv,
	intern:                       bool,
	no_spec_tables:               bool,
	does_regalloc:                bool,
...
qualify_enm :: proc(qual: string, enm: typeid) -> string {
	if enm == backend.Node_Type do return fmt.tprintf("%v%v", qual, enm)
...
generate_spec :: proc(spec_in: Spec_Gen_Input, out_path: string) {
	...
	for classes in spec.classes {
		for &class in classes.ids {
			if class.id == nil do class.id = backend.No_Extra
		}
	}
		...
		for class, i in classes.ids {
			...
			collect_inheritable :: proc(
				...
				if id not_in inheritable {
					assert(
						len(inheritable) <
						size_of(backend.Inherit_Table_Elem) * 8,
					)
	...
	fmt.assertf(
		len(inheritable) <= size_of(backend.Inherit_Table_Elem) * 8,
	...
	if !spec.no_spec_tables {
		...
		for classes in spec.classes {
			for class, i in classes.ids {
				inherit_set: backend.Inherit_Table_Elem

				mark_inherits(&inherit_set, class.id, inheritable)

				mark_inherits :: proc(
					slot: ^backend.Inherit_Table_Elem,
					t: typeid,
					inheritable: map[typeid]int,
				) {
					idx := inheritable[t]
...
@(rodata)
IDEAL_CLASSES := [backend.Node_Type]Class_Spec {
	...
	.Local = {id = backend.Local, args = {"mem"}, default_type = .Void},
...
Class_Spec :: struct {
	...
	group:          string,
	varargs:        bool,
	default_type:   Maybe(backend.Node_Datatype),
	flags:          backend.Class_Flags,
	extra_capacity: int,
	no_ctor:        bool,
	pass_lane:      bool,
}
