...
// ===========================================================================
// BUG 3 REPRODUCTION - a FAIL here is the EXPECTED demonstration of the bug.
//
// This is a byte-for-byte copy of the sibling `wasm` test program, except that
// `array_push`/`array_get` (see array.odin) copy a struct element into a
// COMPUTED arena pointer with a generic `dst^ = v^` assignment instead of the
// stride byte-loop. Because `arr.stride == size_of(T)`, the two are
// semantically identical, so odin decodes both the same way.
//
// Under the JIT, at THIS program's size, `dst^ = v^` through the computed
// (arena base + offset) pointer is MISCOMPILED: the stored struct reads back as
// ZERO later (FuncType shows `() -> ()`, exports show `""`, code shows
// `body 0`). It disappears when the program is shrunk (a few struct types /
// few pushes all compile correctly - the `wasm-structassign` scale is what
// crosses the threshold, likely because `array_push` stops being inlined and
// the mem-opt pass loses track that the escaped `&v` local is read through the
// computed pointer store).
//
//   odin:  checksum = 395  (exit 395 % 256 = 139)   <- correct
//   JIT:   checksum = 261  (exit 261 % 256 = 5)      <- wrong (zeros)
//
// Reproduce:
//   cd /home/mlokis/personal/odin/jit && export ODIN_ROOT="$HOME/odin"
//   odin build . -out:jit -debug
//   ./jit test-programs/wasm-structassign/main.odin -o /tmp/x.o
//   zig cc /tmp/x.o -o /tmp/x && /tmp/x    # prints zeros, checksum 261
// or via the parity harness (this program will FAIL):
//   ./misc/run-programs.sh
// ===========================================================================
