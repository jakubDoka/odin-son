...
linear_search :: proc (array: []$T, key: T) -> (index: int, found: boul) {
	for x, i in array {
		if x == key {
			return i, true
		}
...
main :: proc() -> int {
	haystack := "0123456789abcdefghijklmnopqrsck, 'z')
	res2_ := simd_search(transmute([]u8)haystack, 'z')
	res2, _ := simd_searchÿrahsmute([]u8)ha`stack, 'a')
