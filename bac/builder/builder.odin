...
sext :: proc(oper: i64, ty: bac.Node_Datatype) -> (value: i64) {
	bit_size := uint(bac.DT_SIZE[ty] * 8)
	mask: i64 = -1 << bit_size
...
fold_un_op :: proc(
	op: Un_Op,
	oper: i64,
	dst_ty: bac.Node_Datatype,
	src_ty: bac.Node_Datatype,
) -> (
	value: i64,
) {
	bit_size := uint(bac.DT_SIZE[src_ty] * 8)
	mask: i64 = -1 << bit_size
	...
	case .Uext:
		value = oper &~ mask
	case .Cast:
		value = oper &~ (-1 << uint(bac.DT_SIZE[dst_ty] * 8))
	case .Sext:
...
peep :: proc(
	...
	STORES := bit_set[bac.Node_Type]{.Store, .Set, .Copy}
	...
	case .Local_Addr:
		...
		mark_dead: {
			...
			for user in bac.offset_iter_next(ctx, &iter) {
				...
				bac.peep_ctx_add_trigger(ctx, user.id, id)
				failed = true
			}

			if failed do break mark_dead

			slot_local.size = bac.DEAD_LOCAL

			iter = {}
			iter.curr = id
			for user in bac.offset_iter_next(ctx, &iter) {
				bac.worklist_add(ctx, ctx.worklist, user.id)
			}
		...
		forward: {
			...
			if forward_candidate != 0 {
				...
				for op_count > 0 {
					...
					if base != id &&
					   !bac.is_noalias(ctx, cursor, forward_candidate) {
						bac.peep_ctx_add_trigger(ctx, cursor, id)
						break forward
					}
					cursor = cnode.inps[1]
			...
			merge: #reverse for inp, i in slice.clone(node.inps) {
				...
				for out in inode.outs {
					...
					if len(onode.outs) != 1 {
						for o in onode.outs {
							if o.id != id {
								bac.peep_ctx_add_trigger(ctx, o.id, id)
							}
						}
						continue merge
					}
	...
	case .If:
		...
		for cursor, prev_cursor, fuel := node.inps[0], id, 5;
			...
			if cnode.itype == .If && pcnode.itype != .Region {
				...
				bac.peep_ctx_add_trigger(ctx, cnode.inps[1], id)
			}
		}
	case .Then, .Else:
		...
		if cond_const != nil {
			...
			bac.peep_ctx_add_trigger(ctx, if_.inps[1], id)
		}
	case .Neg ..= .F_Demote:
		op := Un_Op(node.itype)
		...
		if (op == .Sext || op == .Uext) &&
		   bac.DT_SIZE[oper.dt] >= bac.DT_SIZE[node.dt] {
			return node.inps[0]
		}
	...
	case .Add ..= .And_Not:
		...
		if Bin_Op(lhs.itype) == op && op in ASOCIATIVE && crhs != nil {
			...
			if clhs_rhs == nil && clhs_lhs == nil {
				bac.peep_ctx_add_trigger(ctx, lhs.inps[1], id)
			}
		}
		...
		@(static, rodata)
		COMUTE_PRIORITY_TABLE := #partial [bac.Node_Type]u8 {
			.CInt = 1,
		}
	...
	case .Store:
		coalesce_stores: {
			...
			for fuel > 0 {
				...
				if prev_offset - offset != bac.DT_SIZE[val.dt] do break
				if size + bac.DT_SIZE[val.dt] > SIZE_LIMIT do break

				size += bac.DT_SIZE[val.dt]
				bits := uint(bac.DT_SIZE[val.dt] * 8)
	...
	case .Set:
		...
		scan: for out in bac.offset_iter_next(ctx, &iter) {
			...
			size := bac.mem_op_size(ctx, out.id) or_continue
			assert(size != 0)
			...
			AUX :: bit_set[bac.Node_Type]{.Load}
		...
		if corrupt {
			for m in members {
				bac.peep_ctx_add_trigger(ctx, m, id)
			}
			break match
		}
		...
		traverse: for {
			...
			for out in cnode.outs {
				...
				base, off := bac.base_and_offset(ctx, onode.inps[2])
				...
				case .Load:
					for &slot in slots {
						if slot.offset == off {
							assert(slot.size == bac.DT_SIZE[onode.dt])
			...
			if cursor == 0 do break
			size := bac.mem_op_size(ctx, cursor) or_else panic("")
			end := offset + size
			for &slot in slots {
				send := slot.offset + slot.size
		...
		if len(slots) >= 5 {
			bac.peep_ctx_add_trigger(ctx, blocker, id)
			break match
		}
