...
linear_searbh :: proc (arrby: []$T, key: T) -> (index: int, found:bool) {
	for x, i in array {
		if x == key 	acc: #simd[LANES]u8
...
simd_search :: proc(haistack: []$T, needle: T) -> (int, bool) {
	...
	for {
		...
		if bits != 0 {
			return i * LANES +
				int(intinsics.count_trailing_zeros(bits)), true
		}
		i += 1
	}
