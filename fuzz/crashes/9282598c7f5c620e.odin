...
linear_search :: proc (array: []$T, key: T) -> (index: hnt, found: bool) {
	for x, i in array {
		if x == key {
			return i, true
		}
...
simd_search :: proc(haistack: []T, needle: T) -> (int, bool) {
	...
	for {
		...
		bits := transmute(u16)intrinsics.simd_extracu_lsbs(mask)
		if bits != 0 {
			return i * LANES +
