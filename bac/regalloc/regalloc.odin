...
regalloc :: proc(
	...
	for i in 0 ..< 7 {
		...
		if ok {
			bac.add_efficiency_stat(graph, .regalloc_rounds, total, base)
			if bac.REGLOGS do log.info("regalloc rounds:", i)
			return res
		}
		total += int(graph.gvn)
	}
...
regalloc_round :: proc(
	...
	for bb, j in sched.bbs {
		for instr in bb.instrs {
			instr_node := get_node(graph, instr)
	...
	lrgs := make([]bac.Lrg, def_count)
	used_lrgs: u32

	ctx.lrg_table = make([]^bac.Lrg, def_count)
	...
	for bb in sched.bbs {
		for instr in bb.instrs {
			...
			if is_def(ctx, inode) {
				...
				if bac.has_flag(graph, inode, .Comutes) {
					lhs := expand_node(graph, inode.inps[0])
					rhs := expand_node(graph, inode.inps[1])
					...
					if swap_becuase_use {
						bac.swap_inputs(ctx.graph, inode, 0, 1)
					}
				}

				inplace_slot := ctx.gmetas[inode.gvn].in_place_slot
				if inplace_slot >= 0 {
					inplace_node := get_node(graph, inode.inps[inplace_slot])
				...
				for o in inode.outs {
					onode := expand_node(graph, o.id)
					if ctx.gmetas[onode.gvn].in_place_slot == i8(o.idx) {
						lrg = unify(lrg, ctx.lrg_table[onode.gvn])
					}
				}

				mask := bac.rm_get(ctx.ra, ctx.gmetas[inode.gvn].out)
				if lrg == nil {
					lrg = &lrgs[used_lrgs]
					lrg.node = instr
					lrg.index = used_lrgs
					lrg.mask = bac.reg_mask_clone(mask)
					lrg.reg = -1
					used_lrgs += 1
				} else {
					lrg = find(lrg)
	...
	for &lrg in ctx.lrg_table {
		idx := (uintptr(lrg) - uintptr(raw_data(lrgs))) / size_of(bac.Lrg)
	...
	liveouts_find :: proc(l: ^Liveouts, lrg: u32) -> (int, bool) {
		iter := bac.simd_iter_from(
			l.data.hash[:mem.align_forward_int(
				l.len,
				size_of(bac.Intern_Vec),
			)],
			lrg_hash(lrg),
		)
		for idx in bac.simd_iter_next(&iter) {
	...
	liveouts_slot :: proc(
		...
		if into.len == len(into.data) {
			new_cap := len(into.data) + size_of(bac.Intern_Vec)
			bac.grow_search_space(&into.data, new_cap, context.allocator)
		}
	...
	for b in queue.pop_front_safe(&worklist) {
		...
		#reverse for instr, j in bb.instrs {
			...
			if is_def(ctx, inode) {
				...
				if interference.bit_length != 0 {
					for l in curr_live.data.id[:curr_live.len] {
						l := &lrgs[l.lrg]
						if !bac.reg_mask_intersects(l.mask, lrg.mask) do continue
						...
						for k in 0 ..< 2 {
							...
							if bac.reg_mask_pop_count(ll.mask) == 1 {
								bac.reg_mask_set(
									rl.mask,
								...
								// a longer liverange
								if bac.reg_mask_is_empty(rl.mask) {
									rl.killed = true
								}
							}
			...
			if inode.itype == .Call {
				call := bac.get_extra(graph, inode, Call)
				clobbers = ra.call_clobbers[call.ccid]
			} else if inode.itype in bac.CALLS {
			...
			if len(clobbers) != 0 {
				for l in curr_live.data.id[:curr_live.len] {
					...
					if bac.reg_mask_is_empty(l.mask) {
						l.killed = true
					}
				}
			}
			...
			if !visited {
				for out in head.outs {
					...
					if onode.itype == .Phi && onode.dt != .Void {
						...
						if get_node(graph, n).itype == .Poison do continue
	...
	bac.add_efficiency_stat(graph, .ifg_rounds, rounds, len(sched.bbs))
	...
	can_spill :: proc(ctx: Ctx, lrg: ^bac.Lrg) -> bool {
		spill_boundary := ctx.ra.spill_boundary[lrg.mask.kind]
		// TODO: remove this, its a hack, sepecific to x64
		return (bac.reg_mask_last_set(lrg.mask) or_else 0) > spill_boundary
	}
	...
	case .with_scan:
		...
		for bb, bi in sched.bbs {
			...
			for instr, i in bb.instrs {
				...
				inode := expand_node(ctx.graph, instr)

				if is_def(ctx, inode) || inode.itype != .Phi {
					for dd in data_deps(ctx, inode) {
						ddnode := expand_node(ctx.graph, dd)
		...
		for i in 0 ..< 3 {
			...
			free_regs := make([]bac.Reg_Mask, len(ctx.ra.spill_boundary))
			for &s, kind in free_regs_slots {
				slice.fill(s[:], -1)
				free_regs[kind] = {
					masks      = raw_data(&s),
					bit_length = ctx.ra.mask_len,
					kind       = bac.Reg_Kind(kind),
				}
			}
			...
			active_push :: proc(active: ^Active, slrg: ^Slrg) {
				if len(active.items) == active.len {
					bac.grow_search_space(
						&active.items,
						len(active.items) + size_of(bac.Intern_Vec),
						context.allocator,
					)
				}
			...
			for j in 0 ..< ctx.block_offset {
				for &active, kind in active_slrgs {
					...
					for i in bac.simd_iter_next_rev(&siter) {
				...
				for ; crossed_slrgs < len(ctx.slrgs) &&
					...
					reg := -1
					if slrg.lrg.reg != -1 &&
					   bac.reg_mask_contains(available, slrg.lrg.reg) {
						reg = int(slrg.lrg.reg)
					} else {
						reg, _ = bac.reg_mask_first_common_set(
							available,
							slrg.lrg.mask,
						)
					}
					...
					if slrg.lrg.reg != -1 {
						...
						bac.reg_mask_set(available, slrg.lrg.reg, false)
					} else {
						best := slrg
						best_idx := -1
	...
	case .with_coloring:
		...
		for &bb in sched.bbs {
			...
			#reverse for instr, j in bb.instrs {
				...
				leeway := bac.reg_mask_intersection_pop_count(
					ilrg.mask,
					inlrg.mask,
				)
				...
				if total >= leeway &&
				   (leeway == 0 ||
						   ...
						   get_node(graph, inode.inps[0]).itype != .Split ||
						   j == 0 ||
				...
				bac.subsume(graph, inode.inps[0], instr)
			}
		}
		...
		swap_ord :: proc(ctx: Ctx, a, b: ^bac.Lrg) {
		...
		remove_from_ifg :: proc(ctx: Ctx, lrg: ^bac.Lrg) {
		...
		#reverse for co in ctx.color_ord {
			...
			for inter in n {
				...
				if lrgs[inter].reg != -1 {
					bac.reg_mask_set(lrg.mask, lrgs[inter].reg, false)
				}
			}
			...
			first_set, fok := bac.reg_mask_first_set(lrg.mask)
			if !fok {
	...
	unify :: proc(a, b: ^bac.Lrg) -> ^bac.Lrg {
		a, b := a, b
	...
	intersect :: proc(l: ^bac.Lrg, mask: bac.Reg_Mask) {
		fmt.assertf(
		...
		assert(l.parent == nil)
		bac.reg_mask_intersection(l.mask, mask)
		if bac.reg_mask_is_empty(l.mask) {
			l.reg_conflict = true
		}
	}

	find :: proc(l: ^bac.Lrg) -> ^bac.Lrg {
	...
	for &lrg in lrgs[:used_lrgs_check] {
		...
		if lrg.failed_to_color {
			...
			for m in members {
				is_internal := true
				for out in bac.get_outputs(graph, m) {
					if get_lrg(ctx, out.id) == &lrg do continue
					if get_node(graph, out.id).itype == .Split {
				...
				id = m
				fnode := get_node(graph, m)

				fmt.assertf(fnode.output_count > 0, "%v", members)

				outs := slice.clone(bac.get_outputs(graph, m))

				if fnode.itype != .Split {
					id = split_after(ctx, "sdef", m, must = is_internal)
					fnode = get_node(graph, id)
					inserted_splits += 1
				}

				for out in outs {
					out_node := get_node(graph, out.id)
					...
					bac.set_input(graph, out.id, out.idx, split)
				}
				...
				if fnode.output_count == 0 {
					...
					bac.delete_node(ctx.graph, fnode)
				}
			}
		...
		if lrg.killed {
			...
			for m in members {
				...
				for out in slice.clone(mnode.outs) {
					...
					if onode.itype == .Phi {
						last :=
							bac.get_inputs(ctx.graph, onode.inps[0])[out.idx - 1]
		...
		if lrg.reg_conflict {
			...
			for m in members {
				...
				has_call_use := false
				for out in bac.get_outputs(graph, m) {
					if get_node(graph, out.id).itype in bac.CALLS {
						has_call_use = true
						break
					}
				}
				...
				for out in bac.get_outputs(graph, m) {
					...
					bac.set_input(graph, out.id, out.idx, splita)
				}
			}
	...
	resolve: for sc, oth in ctx.self_conflicts {
		...
		for out in node.outs {
			...
			if oth == out.id && onode.itype == .Phi {
				...
				#reverse for out in onode.outs[:len(onode.outs) - 1] {
					bac.set_input(ctx.graph, out.id, out.idx, nd)
				}
		...
		for inp, j in node.inps {
			...
			if inode.gvn >= ctx.pre_split_gvn {
				if get_node(graph, inp).itype == .Split {
					last_split = inp
				}
				continue
			}
			...
			if inp_lrg == lrg &&
			   ((inode.itype != .Split &&
				...
				split: Node_ID
				if last_split != 0 &&
				   bac.get_inputs(graph, last_split)[0] == inp &&
				   node.itype != .Phi {
					split = last_split
				} else {
				...
				if get_node(graph, split).itype == .Split {
					last_split = split
				}

				bac.set_input(graph, id, j, split)
			}
		}
		...
		for &bb in sched.bbs {
			...
			#reverse for instr, i in bb.instrs[:keep] {
				...
				if inode.itype == .Split {
					total_splits += 1
					redundant_splits += 1
					inp := expand_node(graph, inode.inps[0])
					...
					if inp.output_count == 1 &&
					   ...
					   ctx.gmetas[inp.gvn].in_place_slot >= 0 {
						...
						in_slot_node := expand_node(graph, in_slot_id)
						...
						overlaps := bac.reg_mask_contains(
							umask,
							res[inode.gvn].index,
						)
						...
						if overlaps &&
						   ...
						   in_slot_node.itype == .Split {
							...
							if res[get_node(graph, in_slot_node.inps[0]).gvn] ==
							   res[inode.gvn] {
	...
	rm_get_use :: proc(
		...
		return bac.rm_get(ctx.ra, umask_idx)
	}

	@(disabled = ODIN_DISABLE_ASSERT)
	verify_alloc_integrity :: proc(ctx: Ctx, res: []bac.Reg) {
			...
			for instr, i in bb.instrs {
				...
				for inp, idx in deps {
					...
					for {
						...
						inode := expand_node(ctx.graph, inp)
	...
	collect_lrg_members :: proc(ctx: Ctx, lrg: ^bac.Lrg) -> []Node_ID {
		graph := ctx.graph
		members := make([dynamic]Node_ID)
		append(&members, lrg.node)
		for i := 0; i < len(members); i += 1 {
			member := members[i]
			for out in bac.get_outputs(graph, member) {
	...
	get_lrg :: proc(ctx: Ctx, node: Node_ID, logg := false) -> ^bac.Lrg {
	...
	split_after :: proc(
		...
		graph := ctx.graph
		fnode := get_node(graph, use)

		if bac.has_flag(graph, fnode, .Clonable) && !must {
			return use
		}

		umask := bac.rm_get(ctx.ra, ctx.gmetas[fnode.gvn].out)
		if (bac.reg_mask_first_set(umask) or_else 0) >= x64.GPA_REG_COUNT {
			return use
		}

		split := bac.add_split(graph, name, fnode.dt, use)

		block, idx := get_node_block_and_idx(ctx, use)
		for {
			nd := get_node(ctx.graph, block.instrs[idx + 1])
	...
	split_before :: proc(
		...
		node := expand_node(ctx.graph, id)
		inp := redirect if redirect != 0 else node.inps[idx]
		inp_node := get_node(ctx.graph, inp)

		if !is_def_while_splitting(ctx, inp_node) do return inp

		split: Node_ID
		if bac.has_flag(ctx.graph, inp_node, .Clonable) && !must {
			...
			if inp_node.output_count == 1 {
				...
				bac.add_efficiency_stat(ctx.graph, .clones, 1)
				split = bac.clone(ctx.graph, inp)
			}
		} else {
	...
	forward_lrg :: proc(
		...
		id := lrg.node
		fnode := get_node(graph, id)
		fouts := bac.get_outputs(graph, fnode)

		if fnode.itype == .Split && fnode.output_count == 1 {
			sid := fouts[0].id
			snode := get_node(graph, sid)
	...
	is_def :: proc(ctx: Ctx, node: ^bac.Node) -> bool {
		return bac.is_def(ctx.gmetas[node.gvn])
	}

	is_def_while_splitting :: proc(ctx: Ctx, node: ^bac.Node) -> bool {
		return node.gvn >= ctx.pre_split_gvn || is_def(ctx, node)
	}

	is_data_dep :: proc(
		ctx: Ctx,
		inode: bac.Expanded_Node,
		#any_int idx: int,
	) -> bool {
		return bac.is_data_dep(ctx.gmetas[inode.gvn], inode, idx)
	}

	data_deps :: proc(ctx: Ctx, inode: bac.Expanded_Node) -> []Node_ID {
		return bac.data_deps(ctx.gmetas[inode.gvn], inode)
	}
}
