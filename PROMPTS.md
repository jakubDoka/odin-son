...
### Benchmark the new linear scan regalloc

NOTE: read AGENTS.md

Create a autogenerated test-program that will stress test the performance of
the register allocator. It would be great if the generated functions were big
and composed from some set of building blocks that can be randomly selected and
click together, the current test cases are not sufficient because the functions
never really exceed a size for which the registser allocator struggles, and
also thery are repetative so cpu has eazy time predicting it.

The goal it to have a tests that takes at least 1s to compile, with the release
compiler. It should obviously be generated by a script, but the functions it
generates should not be too big or the compiler will likely crash, Lets say the
limmit is 2-4k loc per finction.

Compile the compiler once and reuse it, if the script for generating this is
too slow being written in bash, also use simple odin program to generate it
instead. Basically minimize the time you spend waiting for someting to execute.
Also enforce timeouts fo at most 10 seconds on each command you run. The
compiler could encounter a bug that prevents it from halting and that could
waste time.

If you encounter bugs in the compiler that are unrelated to this test, add a
